...
이 커맨드를 실행할 때 [`architecture` 스킬](../skills/architecture/SKILL.md)의 규칙을 반드시 참조한다.
...
계획에 따라 파일을 재배치한다:
- 엔트리 컴포넌트 (App, Router, Providers) → `app/`
- route 기준 페이지 → `pages/[page-name]/`
- 공통 유틸리티, 훅, UI 컴포넌트 → `shared/`의 적절한 세그먼트
- 정적 자원 → `shared/assets/`
...
도구 설정:
- `tsconfig.app.json` (또는 `tsconfig.json`)에 경로 별칭 추가
...
상세 규칙: [shared-api.md](../skills/architecture/layers/shared-api.md)
...
상세 규칙: [shared-query-factory.md](../skills/architecture/layers/shared-query-factory.md), [shared-mutation-factory.md](../skills/architecture/layers/shared-mutation-factory.md)
...
- Next.js 등 자체 라우팅이 있는 프레임워크는 해당 구조에 맞게 적응한다.

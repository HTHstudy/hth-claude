...
- `--src-dir`는 사용하지 않는다. 루트 `app/`이 Next.js 라우팅 디렉토리가 되고, `src/`는 아키텍처 레이어 전용으로 수동 생성한다.
...
Next.js의 기본 구조를 아키텍처 레이어드 아키텍처로 전환한다. 상세 규칙: [nextjs.md](../../architecture/integrations/nextjs.md)

> **핵심 원칙:** 루트 `app/`은 Next.js 라우팅 전용(re-export만), `src/`는 아키텍처 레이어 전용. 이 두 영역은 반드시 분리한다.
...
2. `src/` 디렉토리 수동 생성 후 아키텍처 레이어 배치:
...
**루트 `app/page.tsx`** — 아키텍처 page re-export:
...
**경로 별칭** — [project-config.md#tsconfig-paths](../../architecture/rules/project-config.md#tsconfig-paths)를 따라 `tsconfig.json`에 `paths`를 설정한다. `create-next-app`이 생성한 기본 `@/*` alias는 제거한다.
...
**`src/shared/config/env.ts`** — [project-config.md#envts](../../architecture/rules/project-config.md#envts)의 Next.js 템플릿을 따라 환경변수 중앙화 모듈을 생성한다.

**`src/shared/routes/paths.ts`** — [project-config.md#pathsts](../../architecture/rules/project-config.md#pathsts)를 따라 라우트 경로 모듈을 생성한다. 신규 프로젝트이므로 빈 `PATHS` 객체로 scaffold한다.

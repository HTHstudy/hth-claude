...
파일명(kebab-case), Named Export, import type 전환은 Phase 1에서 완료되어 있다. Phase 3-4에서 새로 작성한 코드에 잔여 위반이 있을 수 있으며 eslint가 자동 탐지한다.

| 항목 | 점검 내용 | 전환 방법 |
|------|-----------|-----------|
| 파일명 | Phase 1에서 kebab-case 완료 | 잔여 위반이 있으면 `git mv`로 변경 |
| export | Phase 1에서 Named Export 완료 | 잔여 `export default`는 eslint가 탐지 |
| import | Phase 1에서 import type 완료 | 잔여 위반은 eslint가 탐지 |

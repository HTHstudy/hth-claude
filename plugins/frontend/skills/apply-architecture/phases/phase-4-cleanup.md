...
파일을 하나씩 열어 읽지 않는다. 셸 명령으로 위반 목록만 추출한 뒤, 위반 파일만 수정한다:

```bash
# 1. 파일명 — 케밥 케이스 위반 (대문자 포함 파일 탐지)
find src/ -type f \( -name "*.ts" -o -name "*.tsx" \) | grep '[A-Z]'

# 2. export default 사용 파일
grep -rl "export default" src/ --include="*.ts" --include="*.tsx"

# 3. 타입 import에 type 키워드 누락 (import { Type } from 패턴 중 타입만 사용하는 경우)
# → 이 항목은 파일 수정 시 함께 교정한다. 전수 탐지는 비용 대비 효과가 낮으므로 생략 가능.
```

셸 명령의 결과를 위반 목록 표로 정리하여 사용자에게 제시하고, 확인을 받은 후 진행한다.

파일 rename 및 import 수정은 [Phase 1의 일괄 처리 전략](phase-1-structure.md#일괄-처리-전략)을 동일하게 적용한다 (매핑 테이블 → 셸 스크립트 일괄 `git mv`, macOS 대소문자 FS 2단계 rename, import 일괄 치환 후 grep 검증).

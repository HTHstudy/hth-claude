...
### macOS 셸 호환
- `sed -i ''` 사용 (GNU sed의 `-i`와 다름)
- `\b` 워드 바운더리는 macOS sed에서 미지원 → `perl -pi -e` 사용 또는 패턴을 앞뒤 구분자로 대체
- `grep -P` (PCRE)도 macOS 기본 grep에서 미지원 → `grep -E` 사용

### 일괄 처리 우선
- 개별 Read/Edit 반복 대신 sed/perl 일괄 치환 → grep 잔여 검증 패턴 사용

...
Next.js 프로젝트에서는 API route ↔ 아키텍처 레이어 간 import도 차단해야 한다. 위 템플릿의 레이어별 설정에 아래 override를 추가한다. 상세 배경은 [nextjs.md의 SKILL.md 규칙과의 차이 섹션](../integrations/nextjs.md#skillmd-규칙과의-차이)을 참조.

```js
// API route 파일에서 아키텍처 레이어 import 차단
{
      ...
      patterns: [
        {
          ...
          message: 'API route는 아키텍처 레이어를 import할 수 없습니다. 자체적으로 로직을 완결하세요.',
        },
      ],
    }],
  },
...
> 아키텍처 레이어 → API route 방향은 별도 차단 불필요. API route는 `src/` 밖(루트 `app/api/`)에 있어 path alias로 접근할 수 없고, 상대경로도 depth가 깊어 실질적으로 발생하지 않는다.

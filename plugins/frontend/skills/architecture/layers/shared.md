...
- page 문맥에 결합된 코드는 shared에 두지 않는다.
...
shared 모듈은 생성 방식에 따라 두 부류로 나뉜다. Segment 단위가 아니라 **모듈 단위**로 판단한다.

### 4.1 기반 모듈 — 시작부터 shared에 둔다

page보다 먼저 존재해야 하는 프로젝트 기반이다. page에서 추출하는 것이 아니라 프로젝트 시작 시점부터 shared에 생성한다.

- 기반 인프라: `config`, `routes`, `i18n`, `api` — page가 동작하기 위한 인프라
- 디자인 시스템 / 헤드리스 컴포넌트 — 프로젝트가 채택한 UI 기반

### 4.2 이동 모듈 — 사용하는 곳에서 시작하고, 공통이 생기면 shared로 이동한다

business-agnostic 코드라도 처음에는 사용하는 곳에 둔다. 2개 이상의 Slice에서 실제로 공통 사용이 발생하면 shared로 이동한다.

이동 조건 — 다음을 **모두** 만족해야 한다. 하나라도 불충족이면 현재 범위에 남긴다.

1. **2개 이상의 Slice에서 실제로 사용하고 있는가?** (예측이 아닌 실제 사용)
2. Slice 문맥 없이도 의미가 성립하는가?
3. 입력/출력이 특정 Slice 상태에 결합되어 있지 않은가?
4. 해당 책임이 충분히 안정적인가? — 모듈의 인터페이스(props/params/return type)가 최근 변경되었거나 변경 예정이면 불안정으로 판단한다. `git log`로 최근 커밋을 확인하여 인터페이스 변경이 진행 중이면 이동하지 않는다.
5. 범용 props 또는 범용 인터페이스만으로 동작하는가?
...
| `hooks/`            | 범용 커스텀 훅. page 상태에 결합하지 않는 인프라 수준 훅만 둔다                                                                                                                        |
...
- 브랜드 테마 적용
- UI 자체 로직을 포함하는 컴포넌트 (로고, 레이아웃, 자동완성/검색창 등). 단, 로컬 상태/표현 로직만 허용한다. 원격 데이터 조회는 상위 레이어(page 등)가 담당하고 props로 주입한다.
- props만으로 렌더링이 완결되는 컴포넌트

### 금지

- page context, page 전용 hook, page 전용 타입에 의존하는 컴포넌트
- 비즈니스 로직을 포함하는 컴포넌트
...
도메인 특화가 필요하면 page 내부에서 shared 컴포넌트를 감싸서 사용한다.
...
- page 상태나 context를 내부에서 참조하는 훅
- 비즈니스 로직을 포함하는 훅 → 해당 page 또는 feature에 둔다
...
| `ui/`                | 공통 UI, 표현, UI 자체 로직      | `lib`, `i18n`, `routes`(링크류 컴포넌트에 한정) | `api`, `query-factory`, `mutation-factory`  |
...
| 백엔드 연결 | API 클라이언트, 도메인별 요청 함수 (`shared/api`)     | —                                   |
...
- 기반 모듈(design system primitive, pure lib, infra hook, 기반 인프라)은 시작부터 shared에 둔다.
- 이동 모듈은 page에서 시작해서 실제 재사용 + 문맥 독립 + 안정성 확인 후 shared로 이동한다.
...
- page에서 태어난 코드를 실제 재사용 없이 shared로 올리지 않는다.
- page 전용 훅을 두 곳에서 쓴다는 이유만으로 shared로 올리지 않는다.
...
- shared 내부에서 `pages`나 `app`의 모듈을 import하지 않는다.

...
- 특정 사용처의 문맥에 결합된 코드는 shared에 두지 않는다.
...
애플리케이션 코드보다 먼저 존재해야 하는 프로젝트 기반이다. 사용처에서 추출하는 것이 아니라 프로젝트 시작 시점부터 shared에 생성한다.

- 기반 인프라: `config`, `routes`, `i18n`, `api` — 애플리케이션이 동작하기 위한 인프라
- 디자인 시스템 / 헤드리스 컴포넌트 — 프로젝트가 채택한 UI 기반
- (TanStack Query 사용 시) `query-factory`, `mutation-factory` — `api`와 함께 초기부터 shared에 생성

### 4.2 이동 모듈 — 사용하는 곳에서 시작하고, 공통이 생기면 shared로 이동한다

business-agnostic 코드라도 처음에는 사용하는 곳에 둔다. 다른 레이어에서 실제로 공통 사용이 발생하면 shared로 이동한다.

이동 조건 — 다음을 **모두** 만족해야 한다. 하나라도 불충족이면 현재 범위에 남긴다.

1. **2개 이상의 사용처에서 실제로 사용하고 있는가?** — 예측이 아닌 실제 사용. 사용처는 레이어를 넘어도 되고(pages 1 + features 1 등), 같은 레이어 안의 서로 다른 단위여도 된다.
2. **책임이 안정적인가?** — 인터페이스(props/params/return type)가 최근 변경되었거나 변경 예정이면 이동하지 않는다. `git log`로 최근 커밋을 확인한다.
3. **문맥 독립적인가?** — 특정 사용처의 상태·context에 결합되지 않고, 범용 props/인터페이스만으로 동작해야 한다. 도메인 엔티티 타입을 직접 props로 받거나 특정 레이어 상태를 내부에서 참조하면 shared에 두지 않는다.
...
| `hooks/`            | 범용 커스텀 훅. 특정 사용처 상태에 결합하지 않는 인프라 수준 훅만 둔다                                                                                                                 |
...
- 특정 사용처의 context/hook/타입에 의존하는 컴포넌트
- 비즈니스 로직을 포함하는 컴포넌트
...
- 특정 사용처의 상태나 context를 내부에서 참조하는 훅
- 비즈니스 로직을 포함하는 훅 → 해당 사용처 레이어에 둔다
...
- 기반 모듈(기반 인프라, 디자인 시스템 / 헤드리스 컴포넌트)은 시작부터 shared에 둔다.
- 이동 모듈은 사용처에서 시작해서 실제 재사용 + 문맥 독립 + 안정성 확인 후 shared로 이동한다.
...
- 사용처에서 태어난 코드를 실제 재사용 없이 shared로 올리지 않는다.

...
entrypoint는 "Slice 외부 접근의 공개 지점" 개념이다. 실제 형태는 다음과 같다.
...
`_` prefix는 "Slice 외부에서 접근하지 않는다"는 표식이다. 처음부터 만들지 않고, Slice 내부 추출이 실제로 발생한 뒤에만 도입한다. 예: `_ui/`, `_hooks/`, `_context/`, `_lib/`.

### 5.3 Path alias

레이어별 alias만 허용한다.

`@app/*`, `@pages/*`, `@shared/*`, `@widgets/*`, `@features/*`, `@entities/*`

전체 소스를 잡는 포괄적 alias(`@/*` 등)는 사용하지 않는다 — 정적 분석으로 import 방향 검증이 불가능해진다.
...
| 감지 대상 | 조건 | 조치 |
|---|---|---|
| ESLint 아키텍처 규칙 | 항상 (강제) | [eslint-config.md](rules/eslint-config.md)의 템플릿 적용. 설정 파일 부재 시 신규 생성, 존재 시 [병합 절차](rules/eslint-config.md#기존-eslint-설정이-있는-프로젝트)로 추가 |
| 경로 상수 정의 | `src/shared/routes/` 부재 시 생성 여부 확인 | 동의 시 [project-config.md#pathsts](rules/project-config.md#pathsts)에 따라 `shared/routes/` scaffold. 거부 시 진행하되 라우트 관련 코드 작성 시 누락 지적 |
| Next.js 프로젝트 | `package.json`에 `next` 의존성 존재 | [nextjs.md](integrations/nextjs.md) 반드시 읽고 적용 (강제) |
| Next.js App Router + TanStack Query | 위 + 루트 `app/` 폴더 + `package.json`에 `@tanstack/react-query` | [nextjs-rsc-tanstack-query.md](integrations/nextjs-rsc-tanstack-query.md) 반드시 읽고 적용 (강제) |

### 코드 작성 시

- **신규 작성 시** §7 판단 순서로 위치를 결정하고, §4 import 방향 / §5 경계와 접근 / [conventions.md](rules/conventions.md)의 일반 컨벤션을 처음부터 적용한다. `_ui/`, `_hooks/` 같은 private 폴더는 실제 추출이 발생한 뒤에 묶는다.
...
ESLint가 적용된 프로젝트는 자동 차단 항목을 Agent가 별도로 탐지하지 않는다. ESLint 미적용 프로젝트는 자동 차단 항목까지 모두 수동으로 본다. 아래 **수동 리뷰 전용** 항목은 ESLint 적용 여부와 무관하게 항상 검토한다.
...
- [ ] `src/shared/routes/` 생성 — 경로 상수 (라우트가 하나라도 정의되면 필수). 형태는 [project-config.md#pathsts](rules/project-config.md#pathsts) 참조 (단일 `paths.ts`, 도메인별 분리 모두 허용)
- [ ] `src/shared/config/env.ts` 생성 — 환경변수 관리. [project-config.md#envts](rules/project-config.md#envts) 참조
- [ ] `src/main.tsx` — app 바깥 부팅 파일로 유지
- [ ] 기존 assets → `src/shared/assets/`로 이동

**필수 설정:**
- [ ] tsconfig path alias — [project-config.md#tsconfig-paths](rules/project-config.md#tsconfig-paths) 참조 (레이어별 alias만 허용, `baseUrl` 미사용)
...
- [ ] tsconfig path alias가 빌드 도구에서 정상 해석되는지 확인 — Vite 8+: `resolve.tsconfigPaths: true`, Vite 7 이하: `vite-tsconfig-paths` 플러그인, Webpack/Next.js/Turbopack: tsconfig.json `paths` 자동 인식
- [ ] Prettier — [project-config.md#prettier](rules/project-config.md#prettier) 참조 (기존 설정 있으면 pass)
...
- 프로젝트 설정 (Prettier·tsconfig paths·env.ts) → [project-config.md](rules/project-config.md)

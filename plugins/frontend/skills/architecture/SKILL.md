...
| 레이어              | 역할                                                                            | 상세                                            |
| ------------------- | ------------------------------------------------------------------------------- | ----------------------------------------------- |
| `app`               | 애플리케이션 조립·실행. 라우팅, 전역 Provider, 전역 스타일, 초기화 로직.        | [app.md](layers/app.md)                         |
...
| `shared`            | 앱의 기반 구성 요소와 기반 도구. 특정 Slice 문맥 없이 독립적으로 성립하는 코드. | [shared.md](layers/shared.md)                   |
...
### 예외
`app`은 조립·실행 레이어, `shared`는 Segment 구성이므로 Slice를 가지지 않는다.
...
## 5. 네이밍 및 entrypoint 규약

| 항목         | 규칙                                                     | 예시                                                                               |
| ------------ | -------------------------------------------------------- | ---------------------------------------------------------------------------------- |
| 파일명       | 케밥 케이스 (kebab-case)                                 | `page-header.tsx`, `use-page-filters.ts`                                           |
...
- **신규 작성 시** §7 판단 순서로 위치를 결정하고, §4 import 방향 / §5 네이밍 및 entrypoint 규약을 처음부터 적용한다. `_ui/`, `_hooks/` 같은 private 폴더는 실제 추출이 발생한 뒤에 묶는다.

...
**순환 의존 금지.** 레이어·Slice·Segment 어느 단위에서든 순환 의존을 만들지 않는다.
...
**Slice 외부에서는 Slice의 내부 파일을 직접 import하지 않는다.** 각 Slice의 entrypoint만 통해 접근한다.

- 단일 파일 Slice는 그 파일 자체가 entrypoint다 (`pages/home.tsx`).
- 폴더 Slice는 폴더 안의 `index.tsx`(UI가 있으면) 또는 `index.ts`(타입/함수만)가 entrypoint다 (`pages/product-detail/index.tsx`).

`index.tsx`는 **종속 컴포넌트를 조합하는 실제 구현 파일**이다. 내부를 단순히 re-export하기만 하는 래퍼로 쓰지 않는다.
...
`index.ts`는 타입이나 함수만 담는 Slice에서 export를 정리하는 entrypoint 역할을 한다.
...
| 항목 | 규칙 |
|------|------|
| 파일명 | kebab-case (`page-header.tsx`, `use-page-filters.ts`) |
| 컴포넌트 단위 | 하위 종속이 없으면 파일, 있으면 폴더 + `index.tsx` |
| Export | Named Export 기본. Default Export는 프레임워크가 요구할 때만 |
| Import | 타입은 반드시 `type` import (`import type { Foo }`) |

### 규칙의 이유

**파일명을 kebab-case로 강제하는 이유.** macOS 기본 파일시스템은 대소문자를 구분하지 않지만 Linux는 구분한다. PascalCase나 camelCase 파일명은 개발 환경에서 문제없이 돌다가 CI·프로덕션에서 `import` 실패를 일으킬 수 있다. kebab-case로 고정하면 이 위험이 원천 차단된다. 읽을 때도 URL 표기와 같은 규약이라 일관된다.

**Named Export를 기본으로 하는 이유.** IDE 리팩토링이 안전해진다. 이름 변경과 참조 찾기가 안정적으로 동작하며, Default Export처럼 호출부마다 다른 이름을 붙이는 현상을 막는다. 같은 함수가 프로젝트 곳곳에서 다른 이름으로 import되면 rename 리팩토링이 기대대로 퍼지지 않는다. 또한 import 라인에서 어떤 심볼을 가져오는지가 바로 드러나 읽기도 명시적이다.

**`type` import를 강제하는 이유.** 런타임 번들에서 타입 참조를 완전히 제거할 수 있다. 빌드 툴이 값과 타입을 구분 없이 drop할 수 있어 번들 사이즈가 줄고, 타입만 참조하는 경로가 런타임 의존처럼 취급되어 순환 의존을 만드는 현상도 줄어든다. 읽는 입장에서도 "이 심볼이 타입인가 값인가"가 import 라인에서 즉시 드러난다.

**순환 의존을 금지하는 이유.** 모듈 초기화 순서가 결정되지 않는다. 한쪽이 아직 평가되지 않은 상태에서 참조되면 `undefined` export가 발생하고, 이 종류의 버그는 빌드 환경·임포트 순서·지연 평가 여부에 따라 재현이 들쭉날쭉하다. 테스트와 리팩토링 시 의존 그래프 추적이 어려워지는 것도 큰 비용이다. 레이어·Slice·Segment·모듈 어느 단위에서도 순환을 허용하지 않는다.
